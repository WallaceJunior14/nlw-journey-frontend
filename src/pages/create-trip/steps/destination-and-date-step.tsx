...
export function DestinationAndDateStep({
  ...
  const displayedDate = eventStartsAndEndDates && eventStartsAndEndDates.from && eventStartsAndEndDates.to
    ? format(eventStartsAndEndDates.from, "d ' de 'LLL").concat(' até ').concat(format(eventStartsAndEndDates.to, "d ' de 'LLL"))
    : null
      ...
      {isDatePickerOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
          <div className=" rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
            ...
            <DayPicker mode="range" selected={eventStartsAndEndDates} onSelect={setEventStartsAndEndDates}/>
          </div>
        </div>
      )}
